...
class MLDecisionEngine:
    ...
    def __init__(self):
        ...
        if self._fallback_active:
            logger.info("ML 决策引擎: 降级到人工权重模式 — 尝试自动训练...")
            try:
                success = self.retrain_if_needed(force=True)
                if success:
                    self._fallback_active = False
                    logger.info("✅ ML 决策引擎: 自动训练成功，切换为主管线")
                else:
                    self._fallback_reason = "训练数据不足或模型文件缺失"
                    logger.warning(f"ML 决策引擎: 自动训练失败 — {self._fallback_reason}")
            except Exception as train_err:
                self._fallback_reason = f"训练异常: {train_err}"
                logger.error(f"ML 决策引擎: 自动训练异常 — {self._fallback_reason}")
        else:
