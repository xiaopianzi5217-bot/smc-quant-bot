    ...
    def decide(
        ...
        if not regime['allowed']:
            return {
                ...
                'observer_plans': observer_plans,
                'candidates': [],
                'primary': None,
                'risk_plan': None,
            }

        candidates = self.signal_gate.collect_candidates(
            macro_ctx, exec_ctx, long_score, short_score, long_threshold, short_threshold, long_reasons, short_reasons
        )
        ...
        risk_plan = self.risk.build_plan(
            primary['direction'], curr, exec_ctx, equity=equity, level=primary.get('level', 'A')
        )
        if risk_plan.get('position') and not risk_plan['position'].get('allowed', True):
            return {
                'approved': False,
                'state': 'REJECTED',
                'reason_cn': risk_plan['position'].get('reason_cn'),
                'regime': regime,
                'observer_items': observer_items,
                'observer_plans': observer_plans,
                'candidates': ranked,
        ...
        quality = risk_plan.get('quality') or {}
        return {
            'approved': True,
            'state': 'APPROVED',
            'reason_cn': '通过v6统一决策内核',
            'regime': regime,
            'observer_items': observer_items,
            'observer_plans': observer_plans,
            'candidates': ranked,
            ...
            'risk_plan': risk_plan,
            'execution': exe,
            'quality_tags': quality.get('tags', []),
            'is_probe': bool(quality.get('is_probe')),
            'quality_size_mult': quality.get('size_mult', 1.0),
        }
