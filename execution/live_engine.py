...
class LiveExecutionEngine:
    def __init__(self, cfg, exchange_adapter, portfolio, logger, notifier=None,
        ...
        Args:
            ...
            logger: 日志对象
            notifier: 通知器（可选）
            feeder: MicroFeeder 实例（可选），传入后启用微观卫士
        """
        self.cfg = cfg
        ...
        # ===== 微观执行卫士 =====
        self.micro_guard: Optional[MicroExecutionGuard] = None
        if feeder is not None and MicroExecutionGuard is not None:
            micro_cfg = cfg.get("micro_guard", {})
            if micro_cfg.get("enabled", False):
                try:
                    self.micro_guard = MicroExecutionGuard(
                        shared_state=feeder.state,
                    ...
                    self.logger and self.logger.log(
                        "MICRO_GUARD",
                        message=f"微观卫士已启动 (shared_state模式)",
                        raw={"thresholds": {
                    ...
                    self.logger and self.logger.log(
                        "MICRO_GUARD", message=f"微观卫士初始化失败: {exc}"
                    )
                    self.micro_guard = None
            else:
                self.logger and self.logger.log(
                    "MICRO_GUARD", message="微观卫士未启用 (micro_guard.enabled=false)"
                )
        else:
            self.logger and self.logger.log(
                "MICRO_GUARD", message="微观卫士未启用 (未传入 feeder 或模块不可用)"
            )
    ...
    def execute_decision(self, symbol, decision):
        ...
        # ===== 微观执行卫士：发单前的盘口验证（直接读 shared_state）=====
        if self.micro_guard is not None:
