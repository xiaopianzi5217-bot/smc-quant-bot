# -*- coding: utf-8 -*-
"""Fix the tail of alpha_master_engine.py - replace from 'def allocate' to EOF"""
...
tail = '''    def allocate(self, signal: Dict[str, Any], risk: float, regime: str, vol_state: str) -> Tuple[str, float]:
    ...
    def _build_verdict(
        ...
        if fvg_dir != "None" and fvg_mid is not None:
            ...
            fvg_str = f"{label_fvg}: {_fvg_mid:.2f}"
            imbalance_parts.append(fvg_str)
        ...
        dir_emoji = "📈" if notional_dir == "Long" else "📉"
        ...
        if allow:
            ...
            lines = []
            lines.append(f"━━━ [信号单] {dir_emoji} {dir_cn} {imbalance_str} ━━━")
...
print(f'Syntax OK. Total length: {len(new_content)} chars')
