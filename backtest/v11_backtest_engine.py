...
def run_backtest(
    ...
    slog.info(f"[回测] 数据: {len(df_exec)} 条 15M K 线, {len(df_macro)} 条 1H K 线")
    slog.info(f"[回测] 范围: {df_exec['datetime'].min()} ~ {df_exec['datetime'].max()}")
    ...
    SCAN_STEP = 4
    slog.info(f"[回测] 开始逐 K 线回放（每 {SCAN_STEP} 根 K 线扫描一次）...")

    for i in range(warmup, total_bars):
        if i % 5000 == 0:
            slog.info(f"[回测] 进度: {i}/{total_bars} ({100*i/total_bars:.0f}%)")
    ...
    slog.info(f"[回测] 完成: {len(trades)} 笔交易, {scan_count} 次扫描")
...
def save_backtest_trades(trades: List[Dict[str, Any]]):
    ...
    slog.info(f"[回测] 已保存 {len(trades)} 笔交易到 {FEATURE_STORE_PATH}")
...
def run_full_backtest(
    ...
    print(f"  回测结果")
    print(f"{'='*60}")
    slog.info(f"  总交易: {result['total_trades']}")
    slog.info(f"  胜率: {result['win_rate']*100:.1f}%")
    slog.info(f"  平均 R: {result['avg_r']:.4f}")
    slog.info(f"  总 R: {result['total_r']:.4f}")
    slog.info(f"  Sharpe: {result['sharpe']:.4f}")
    slog.info(f"  Profit Factor: {result['profit_factor']:.4f}")
    slog.info(f"  最大连续亏损: {result['max_consecutive_losses']}")
    slog.info(f"  平均持仓 bars: {result['avg_bars_held']:.1f}")

    if result.get("by_exit_reason"):
        print(f"\n  退出类型分布:")
        for reason, stats in result["by_exit_reason"].items():
            slog.info(f"    {reason}: {stats['count']} 笔, avg_r={stats['avg_r']:.4f}, wr={stats['win_rate']*100:.1f}%")
    ...
    if auto_calibrate and result["total_trades"] >= 10:
        ...
        try:
            ...
            if cal_result.get("note") == "CALIBRATED":
                slog.info(f"[回测] 参数优化: threshold={cal_result['threshold']}, min_rr={cal_result['min_rr']}")
                result["calibrated_params"] = {
                    "threshold": cal_result["threshold"],
                    "min_rr": cal_result["min_rr"],
                }
        except Exception as e:
            slog.error(f"[回测] 自适应调参异常: {e}")
