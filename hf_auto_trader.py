...
async def scan_and_decide(symbol: str) -> dict | None:
    ...
    if trades is None or trades.empty:
        ...
        return {
            ...
            "atr": _atr_val,  # 【修复20260913】与 SL/TP 重算共用同一 ATR 值（含 max(entry*0.0025) 保护）
    ...
    return {
        ...
        "regime": _regime_name,  # 【修复20260913】改用 HTF 检测值（RANGE/BULL/BEAR），不再用回测引擎的 mixed
...
def evaluate_signal_v6_routing(result: dict) -> dict:
    ...
    # 【修复20260913】路由分级用原始分（保持 A/B/C 判定不变），
    # 但保留 FeedbackLoop 加权分单独存储，供持仓字典/快照透传真实决策分
    if "v6_weighted_score" not in result and result.get("_feedback_result"):
        result["v6_weighted_score"] = round(float(result["_feedback_result"]["weighted_score"]), 2)
    if score <= 0.0:
...
def check_and_open_v6_with_routing(result: dict) -> bool:
    ...
    try:
        ...
        position_manager.update(symbol, {
            ...
            "sl_hit": False,
            "last_sl_msg": "",
            "score": float(result.get("v6_weighted_score", result.get("v6_final_score", score))),  # 【修复20260913】优先写 FeedbackLoop 加权分
...
def check_and_open(result: dict | None) -> bool:
    ...
    # 当历史样本不足时 = model_ev
        # 【修复20260913】EV 优先用 FeedbackLoop 校准值，不再被 blended_ev 覆盖
    # 根因：result["expected_value"] 已改为 _fb_result["ev"]，但此处旧逻辑
    #      又把 ev 重设为 blended_ev（Statistical EV），导致 -0.0929 被 -0.0035 覆盖
    ...
    # ===== 【修复20260913】FeedbackLoop 熔断必须前置到路由开仓之前 =====
