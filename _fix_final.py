...
assert idx >= 0, 'cannot find allocate'

head = content[:idx]

# Build the tail programmatically to avoid nesting issues
import textwrap

tail_funcs = textwrap.dedent("""
    def allocate(self, signal, risk, regime, vol_state):
        ev = safe_float(signal.get("expected_value"), -9.0)
        grade = signal.get("ev_grade", grade_from_expected_value(ev))
        if ev > 0.25: book, mult = "CORE", 1.00
        elif ev > 0.15: book, mult = "TACTICAL", 0.78
        elif ev >= 0.05: book, mult = "SCALP", 0.65
        elif ev >= 0.0: book, mult = "PROBE", 0.42
        else: book, mult = "DUMPSTER", 0.08
        if grade == "S_EV_HOT": mult *= 1.08
        if regime == "CHOP": mult *= 0.82
        if vol_state == "HIGH_VOL": mult *= 0.80
        return book, max(0.0, min(self.max_position_mult, risk * mult))
""")

# Write the tail file separately
with open('_tail_source.py', 'w', encoding='utf-8') as f:
...
def _build_verdict(self, allow, reason, regime, vol_state, signal,
    ...
    kline_color = "白色 \u26aa (衰竭)" if sqz_white else ("绿色 \U0001f7e2 (多头)" if mom > 0 else "红色 \U0001f534 (空头)")
