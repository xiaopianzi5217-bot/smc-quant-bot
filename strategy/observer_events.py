...
def _safe_bool(v: Any) -> bool:
    ...
    if isinstance(v, str):
        return v.strip().lower() in {"1", "true", "yes", "y", "on", "true"}
    try: return bool(v)
    except: return False
...
def detect_observer_events(curr: Any, exec_ctx: Dict[str, Any], long_score: float = 0.0, short_score: float = 0.0) -> List[Dict]:
    events = []
    close = _safe_float(getattr(curr, 'close', None) or exec_ctx.get("close", 0))

    # SQZMOM 变白
    if _safe_bool(exec_ctx.get("sqzmom_white_reversal_long")):
        events.append({"type": "SQZMOM_WHITE", "dir": "Long", "desc": "SQZMOM K线变白（多头动量衰竭）", "key": "sqz_white_long", "strength": long_score})
    if _safe_bool(exec_ctx.get("sqzmom_white_reversal_short")):
        events.append({"type": "SQZMOM_WHITE", "dir": "Short", "desc": "SQZMOM K线变白（空头动量衰竭）", "key": "sqz_white_short", "strength": short_score})

    # 背离 R
    if _safe_bool(exec_ctx.get("has_bot_div")):
        events.append({"type": "DIVERGENCE_R", "dir": "Long", "desc": f"底背离 R 出现 (强度 {long_score:.1f})", "key": "div_bot"})
    if _safe_bool(exec_ctx.get("has_top_div")):
        events.append({"type": "DIVERGENCE_R", "dir": "Short", "desc": f"顶背离 R 出现 (强度 {short_score:.1f})", "key": "div_top"})

    # 接近 OB
    if _safe_bool(exec_ctx.get("near_bullish_ob")):
        events.append({"type": "NEAR_OB", "dir": "Long", "desc": "价格接近 Bullish OB（主力建仓区）", "key": "ob_bull"})
    if _safe_bool(exec_ctx.get("near_bearish_ob")):
        events.append({"type": "NEAR_OB", "dir": "Short", "desc": "价格接近 Bearish OB（主力建仓区）", "key": "ob_bear"})

    # Liquidity Sweep
    if _safe_bool(exec_ctx.get("is_bsl_swept")):
        events.append({"type": "LIQUIDITY_SWEEP", "dir": "Short", "desc": "Buyside Liquidity 被扫（止损猎杀）", "key": "bsl_sweep"})
    if _safe_bool(exec_ctx.get("is_ssl_swept")):
        events.append({"type": "LIQUIDITY_SWEEP", "dir": "Long", "desc": "Sellside Liquidity 被扫（止损猎杀）", "key": "ssl_sweep"})
